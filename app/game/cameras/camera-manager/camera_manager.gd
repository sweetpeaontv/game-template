...
func is_examine_active() -> bool:
	return camera_state == CameraState.EXAMINE \
		or (camera_state == CameraState.TRANSITION and active_rig == RigType.EXAMINE)
...
func add_look_delta(relative: Vector2) -> void:
	if not _look_input_enabled \
		or camera_state == CameraState.TRANSITION \
		or camera_state == CameraState.EXAMINE:
		return

...
var examining: Interactable = null
...
func on_init(new_peer_id: int, new_snapshot: Dictionary) -> void:
	...
	self.name = "Player_%d" % peer_id
	self.config = CharacterConfig.new(new_snapshot)
...
func _setup() -> void:
	...
	if local_player:
		...
		character_model.visible = (camera_type == CameraType.THIRD_PERSON)
		nameplate.visible = false

		InputModeManager.set_input_mode(Input.MOUSE_MODE_CAPTURED)
		UIManager.ui_shown.connect(_on_ui_shown)
	else:
...
func _exit_tree() -> void:
	if local_player and UIManager.ui_shown.is_connected(_on_ui_shown):
		UIManager.ui_shown.disconnect(_on_ui_shown)
#===================================================================================#
...
func _handle_camera_rotation(delta: float) -> void:
	# CharacterModel:transform is server-authoritative synced rollback state, so this must gate on
	# synced state too. camera_manager.is_examine_active() is local-only: the server never sees a
	# remote player's examine/customizer camera, so it would keep rotating their model and spin it
	# on clients. player_status and examining_key are synced, so the server agrees with the client.
	if player_status != PlayerStatus.PLAYING:
		return
	if is_examining() or camera_manager.is_examine_active():
		return
	if camera_input.camera_basis != _previous_camera_basis:
		_previous_camera_basis = camera_input.camera_basis
		character_model.rotate_toward_camera(delta, camera_input.camera_basis)
#===================================================================================#
...
func _handle_movement(delta: float) -> void:
	...
	_update_locomotion_animation(direction)

func _update_locomotion_animation(direction: Vector3) -> void:
	if not is_on_floor() or not _locomotion_input_allowed():
		return

	if direction.is_zero_approx():
		character_model.set_locomotion_animation(character_model.LocomotionAnimation.IDLE)
	elif input.shift_pressed:
		character_model.set_locomotion_animation(character_model.LocomotionAnimation.RUN)
	else:
		character_model.set_locomotion_animation(character_model.LocomotionAnimation.WALK)
...
func _handle_holding_sync() -> void:
	if holding_key == 0 and holding != null:
		...
		return
	if holding_key != 0 and holding == null:
		var new_holding = RegistryLibrary.pickupables.get_entry(holding_key)
...
func _handle_operate(is_fresh: bool) -> void:
	var target = focus_sensor.focus

	if target is Operable:
		if local_player and is_fresh:
			target.interact(self, InteractionTypes.OperableData.new(target.default_operate_action), is_fresh)
			ClientManager.client_relay.make_request(
				CommandRegistry.Command.INTERACT,
				Payload.Interact.create(
					InteractionTypes.InteractionType.OPERABLE,
					target.default_operate_action,
					target.key,
					{},
				),
			)
...
func _handle_examine_sync() -> void:
	if examining_key == 0 and examining != null:
		examining = null
	if examining_key != 0 and examining == null:
		var new_examining = RegistryLibrary.examinables.get_entry(examining_key)
		if new_examining:
...
func _open_esc_menu() -> void:
	await UIManager.push_world_blur()
...
func _on_ui_shown(ui_name: String) -> void:
	if ui_name != "InGameCustomizer":
		return

	var customizer := UIManager.get_ui("InGameCustomizer")
	if customizer == null:
		return

	customizer.exit_customizer.connect(_on_ingame_customizer_exit, CONNECT_ONE_SHOT)
	character_model.visible = true
	var anchor := character_model.get_node_or_null("CoreAP") as Node3D
	camera_manager.transition_to(CameraManager.RigType.EXAMINE, anchor)

func _on_ingame_customizer_exit() -> void:
	character_model.visible = (camera_type == CameraType.THIRD_PERSON)
	var return_rig := CameraManager.RigType.FIRST_PERSON if camera_type == CameraType.FIRST_PERSON else CameraManager.RigType.THIRD_PERSON
	var return_anchor := camera_anchor_fp if camera_type == CameraType.FIRST_PERSON else camera_anchor_tp
	camera_manager.transition_to(return_rig, return_anchor)
#===================================================================================#
...
# API
#===================================================================================#
func set_config(injected_config: CharacterConfig) -> void:
...
func get_examining() -> ExaminableRollback:

...
func spawn_players(players: Array, target_peer_ids: Array = []) -> void:
	...
	for player_data in players:
		...
		var snapshot = player_data.get("snapshot", {})

		if peer_id == 0:
			continue

		# Spawn on server first (authoritative)
		await _spawn_player_impl(peer_id, position, snapshot)
...
func spawn_late_joiner(client_peer_id: int, client_snapshot: Dictionary) -> void:
	"""
	...
	var new_player_data = [{"peer_id": client_peer_id, "position": new_player_position, "snapshot": client_snapshot}]
...
func _spawn_player_impl(peer_id: int, spawn_position: Vector3, snapshot: Dictionary) -> void:
	"""
	...
	player.config = CharacterConfig.new(snapshot)
